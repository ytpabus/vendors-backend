...
function App() {
  ...
  <table className="record-table vendor-new">
    <thead>
      <tr>
        {fields
          .filter(f => f.target === 'vendor' && f.key !== 'x_studio_supplier_order')
          .sort((a, b) => a.position - b.position)
          .map(field => (
    ...
    <tbody>
      {record.vendors.map((vendor, vIdx) => {
        const isEditing = editingVendors[vendor.id];
        return (
          <tr key={vIdx}>
            {fields
              .filter(f => f.target === 'vendor' && f.key !== 'x_studio_supplier_order')
              .sort((a, b) => a.position - b.position)
              .map(field => {
                const value = isEditing ? editingVendors[vendor.id][field.key] : vendor[field.key];
                let style = {};
            ...
            .map(field => (
              <td key={field.key}>
                <input
                  ...
                  onChange={(e) =>
                    handleVendorChange(record.id, field.key, e.target.value)
                  }
                />
              </td>
            ))}
